  ...
  OLD_ROUTES.forEach(route => {
    ...
    route.schedule.forEach(entry => {
      ...
      if (lastIsHada && !firstIsHada) {
        toHada.push({ time: entry.time, stops: entry.stops, source: routeLabel });
      } else if (firstIsHada) {
        fromHada.push({ time: entry.time, stops: entry.stops, source: routeLabel });
      }
    });
  });
  // Sort by time
  const parseTime = t => { const m = t.match(/^(\d{1,2}):(\d{2})$/); return m ? parseInt(m[1]) * 60 + parseInt(m[2]) : 9999; };
...
function renderHadaContent() {
  ...
  // To חד"א card
  if (toHada.length > 0) {
    const toTimesHtml = toHada.map(t =>
  ...
  if (fromHada.length > 0) {
    const fromTimesHtml = fromHada.map(t =>
      `<div class="dep-chip dep-chip--hada"><span class="dep-chip-time">${esc(t.time)}</span><span class="dep-chip-note">${esc(t.source)}</span></div>`
    ).join('');
...
function renderOldRouteTabs() {
  ...
  container.innerHTML = OLD_ROUTE_TABS.map(tab =>
...
function renderScheduleEntry(entry) {
  ...
  if (entry.type === 'נסיעה' && entry.stops) {
    const stopsHtml = entry.stops.map((stop, i) => {
      const hada = isHadaStop(stop) ? ' stop-hada' : '';
      return `<div class="stop-item${hada}"><span class="stop-num">${i + 1}</span>${esc(stop)}</div>`;
    }).join('');
...
function renderOldRouteContent() {
  ...
  const scheduleHtml = route.schedule.map(entry => renderScheduleEntry(entry)).join('');
